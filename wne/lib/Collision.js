...
"use strict";
	...
	scope.merger( scope ,{
		...
		// }
		checkEntitiesCollide : function(entities, gridSize, gridCol) {

			var grid = {};
			for( var e = 0; e < entities.length; e++ ) {
				var entityA = entities[e];
				
				if (!entityA.collidable){
					continue;
				}

				var box1=entityA.getHitBox();
				var	colMin = Math.floor( box1[0]/gridSize ) ,
					rowMin = Math.floor( box1[1]/gridSize ) ,
					colMax = Math.floor( box1[2]/gridSize ) ,
					rowMax = Math.floor( box1[3]/gridSize ) ;
					...
					for( var col = colMin; col <= colMax; col++ ) {
						...
						if( !group ) {
							...
							for( var c = 0, len=group.length; c<len; c++ ) {
								var entityB=group[c];
								var box2=entityB.getHitBox();
								if( !checked[entityB.id] 
									&& entityA.isCollidedWith(entityB) ) {
									entityA.onCollided(entityB);
									checked[entityB.id] = true;
								}
							}
							group.push(entityA);
						}
		...
		checkMoveX : function(box, dx, moveBox){
			if (box[2]<=moveBox[0]){
				dx=box[2]+dx-moveBox[0];
			}else if (box[0]>=moveBox[2]){
				dx=box[0]+dx-moveBox[2];
			}
			return dx;
		},
		...
		checkMoveY : function(box, dy, moveBox){
			if (box[3]<=moveBox[1]){
				dy=box[3]+dy-moveBox[1];
			}else if (box[1]>=moveBox[3]){
				dy=box[1]+dy-moveBox[3];
			}
			return dy;
		}
